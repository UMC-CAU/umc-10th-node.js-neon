...
export const addUser = async (data: any): Promise<number | null> => {
  ...
  try {
    ...
    const [result] = await pool.query<ResultSetHeader>(
      `INSERT INTO member (email, password, name, gender, birthday, address, detail_address, phone_number) VALUES (?, ?, ?, ?, ?, ?, ?, ?);`,
      [
        data.email,
        data.password,
        data.name,
...
export const setPreference = async (
  ...
  try {
    await pool.query(
      `INSERT INTO user_like_food (category_id, user_id) VALUES (?, ?);`,
...
export const getUserPreferencesByUserId = async (
  ...
  try {
    const [preferences] = await pool.query<RowDataPacket[]>(
      "SELECT ulf.id, ulf.category_id, ulf.user_id, fcl.name " +
        "FROM user_like_food ulf JOIN category fcl on ulf.category_id = fcl.id " +
        "WHERE ulf.user_id = ? ORDER BY ulf.category_id ASC;",
      [userId],
    );
